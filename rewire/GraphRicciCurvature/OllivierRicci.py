...
import torch
import pandas as pd
# torch.multiprocessing.set_start_method('spawn') # 通常在主脚本中设置
import multiprocessing as mp
from functools import lru_cache

# import networkit as nk # 不再需要
import networkx as nx
import numpy as np
import ot

from .util import logger, set_verbose, cut_graph_by_cutoff, get_rf_metric_cutoff # 假设这些函数在 .util 模块中可用

EPSILON = 1e-7  # to prevent divided by zero

# ---Shared global variables for multiprocessing used.---
# _Gk = nk.graph.Graph() # 移除
_alpha = 0.5
_weight = "weight"
...
_apsp = {}
_G_nx = nx.Graph() # 新增：存储 NetworkX 图的全局变量
_nx_nodes_to_idx = {} # 新增：NetworkX 节点到索引的映射 (用于 _apsp 索引)
_nx_idx_to_nodes = {} # 新增：索引到 NetworkX 节点的映射 (用于 _apsp 索引)
# -------------------------------------------------------

@lru_cache(_cache_maxsize)
def _get_single_node_neighbors_distributions(node_idx, direction="successors"):
    """Get the neighbor density distribution of given node `node_idx`.

    Parameters
    ----------
    node_idx : int
        Node index corresponding to NetworkX graph `_G_nx` via `_nx_nodes_to_idx`.
    ...
    """
    node = _nx_idx_to_nodes[node_idx]
    # --- Use NetworkX methods ---
    if _G_nx.is_directed():
        if direction == "predecessors":
            neighbors = list(_G_nx.predecessors(node))
        else:  # successors
            neighbors = list(_G_nx.neighbors(node))
    else:
        neighbors = list(_G_nx.neighbors(node))
    # ----------------------------
    ...
    for nbr in neighbors:
        nbr_idx = _nx_nodes_to_idx[nbr]
        if direction == "predecessors":
            # w = _base ** (-_Gk.weight(nbr, node) ** _exp_power)
            w = _base ** (-_G_nx.edges[nbr, node][_weight] ** _exp_power)
        else:  # successors
            # w = _base ** (-_Gk.weight(node, nbr) ** _exp_power)
            w = _base ** (-_G_nx.edges[node, nbr][_weight] ** _exp_power)

        if len(heap_weight_node_pair) < _nbr_topk:
            heapq.heappush(heap_weight_node_pair, (w, nbr_idx)) # Store index
        else:
            heapq.heappushpop(heap_weight_node_pair, (w, nbr_idx))
    ...
    nbr_idx_list = [x[1] for x in heap_weight_node_pair]
    return distributions + [_alpha], nbr_idx_list + [node_idx]


def _distribute_densities(source_idx, target_idx):
    ...
    ----------
    source_idx : int
        Source node index corresponding to NetworkX graph `_G_nx` via `_nx_nodes_to_idx`.
    target_idx : int
        Target node index corresponding to NetworkX graph `_G_nx` via `_nx_nodes_to_idx`.
    Returns
    -------
    x : (m,) numpy.ndarray
    ...
    if _G_nx.is_directed():
        x, source_topknbr = _get_single_node_neighbors_distributions(source_idx, "predecessors")
    else:
        x, source_topknbr = _get_single_node_neighbors_distributions(source_idx, "successors")

    # Distribute densities for target and target's neighbors as y
    y, target_topknbr = _get_single_node_neighbors_distributions(target_idx, "successors")
    ...
    if _shortest_path == "pairwise":
        d = []
        for src_idx in source_topknbr:
            tmp = []
            for tgt_idx in target_topknbr:
                # length = _source_target_shortest_path(src, tgt)
                length = _source_target_shortest_path(_nx_idx_to_nodes[src_idx], _nx_idx_to_nodes[tgt_idx])
                tmp.append(length)
            d.append(tmp)
        d = np.array(d)
    else:  # all_pairs
        # d = _apsp[np.ix_(source_topknbr, target_topknbr)]  # transportation matrix
        # Use the indices directly to access the precomputed matrix
        d = _apsp[np.ix_(source_topknbr, target_topknbr)]  # transportation matrix
...
def _source_target_shortest_path(source, target):
    """Compute pairwise shortest path from `source` to `target` via NetworkX.

    Parameters
    ----------
    source : node
        Source node in NetworkX graph `_G_nx`.
    target : node
        Target node in NetworkX graph `_G_nx`.
    ...
    try:
        ...
        length = nx.shortest_path_length(_G_nx, source=source, target=target, weight=_weight)
    except nx.NetworkXNoPath:
        logger.warning(f"No path found between {source} and {target}, assigning infinite distance.")
        return float('inf')
    # assert length < 1e300, "Shortest path between %d, %d is not found" % (source, target)
    assert length < 1e300, f"Shortest path between {source}, {target} is not found or infinite"
    return length


def _get_all_pairs_shortest_path():
    """Pre-compute all pairs shortest paths of the assigned graph `_G_nx`."""
    logger.trace("Start to compute all pair shortest path.")

    global _G_nx, _nx_nodes_to_idx, _nx_idx_to_nodes

    t0 = time.time()
    # Generate dictionary mapping nodes to consecutive integer indices
    _nx_nodes_to_idx = {node: idx for idx, node in enumerate(_G_nx.nodes())}
    _nx_idx_to_nodes = {idx: node for node, idx in _nx_nodes_to_idx.items()}

    # Compute all pairs shortest paths using NetworkX
    # Use dijkstra_path_length which explicitly supports the weight parameter
    apsp_dict = dict(nx.all_pairs_dijkstra_path_length(_G_nx, weight=_weight))

    # Convert the dictionary of dictionaries to a dense numpy array
    num_nodes = len(_G_nx.nodes())
    apsp_matrix = np.zeros((num_nodes, num_nodes))
    for node1, dists in apsp_dict.items():
        idx1 = _nx_nodes_to_idx[node1]
        for node2, dist in dists.items():
            idx2 = _nx_nodes_to_idx[node2]
            apsp_matrix[idx1, idx2] = dist

    logger.trace("%8f secs for all pair by NetworkX." % (time.time() - t0))
...
def _sinkhorn_distance(x, y, d):
    ...
    """
    # x, y = _parse_to_tensor(x, y) # Missing 'd' argument in original call
    x, y, d = _parse_to_tensor(x, y, d) # Parse all arguments to tensors
    t0 = time.time()
...
def _average_transportation_distance(source_idx, target_idx):
    ...
    """
    source = _nx_idx_to_nodes[source_idx]
    target = _nx_idx_to_nodes[target_idx]

    t0 = time.time()
    # --- Use NetworkX methods ---
    if _G_nx.is_directed():
        source_nbr_nodes = list(_G_nx.predecessors(source))
    else:
        source_nbr_nodes = list(_G_nx.neighbors(source))
    target_nbr_nodes = list(_G_nx.neighbors(target))

    source_nbr_indices = [_nx_nodes_to_idx[nbr] for nbr in source_nbr_nodes]
    target_nbr_indices = [_nx_nodes_to_idx[nbr] for nbr in target_nbr_nodes]
    # ----------------------------

    share = (1.0 - _alpha) / (len(source_nbr_nodes) * len(target_nbr_nodes))
    cost_nbr = 0
    # cost_self = _alpha * _apsp[source][target] # This would not work with indices
    cost_self = _alpha * _apsp[source_idx][target_idx]

    for src_idx in source_nbr_indices:
        for tgt_idx in target_nbr_indices:
            # cost_nbr += _apsp[src][tgt] * share
            cost_nbr += _apsp[src_idx][tgt_idx] * share
    ...
    logger.debug("%8f secs for avg trans. dist. \t#source_nbr: %d, #target_nbr: %d" % (time.time() - t0,
                                                                                       len(source_nbr_nodes),
                                                                                       len(target_nbr_nodes)))
    return m
...
def _compute_ricci_curvature_single_edge(source, target):
    ...
    ----------
    source : node
        Source node in NetworkX graph `G`.
    target : node
        Target node in NetworkX graph `G`.

    Returns
    -------
    result : dict[(node, node), float]
    ...
    # if _Gk.weight(source, target) < EPSILON: # This won't work with NetworkX directly here
    if _G_nx.edges[source, target][_weight] < EPSILON:
    ...
    # Convert node names to indices for internal processing
    source_idx = _nx_nodes_to_idx[source]
    target_idx = _nx_nodes_to_idx[target]

    x, y, neighbors_x_idx, neighbors_y_idx, d = _distribute_densities(source_idx, target_idx)
    optimal_plan = ot.emd(x, y, d)
    optimal_cost = optimal_plan * d
    optimal_total_cost = np.sum(optimal_cost)

    # Convert neighbor indices back to node names for the DataFrame
    neighbors_x = [_nx_idx_to_nodes[idx] for idx in neighbors_x_idx]
    neighbors_y = [_nx_idx_to_nodes[idx] for idx in neighbors_y_idx]
    ...
    result = 1 - (optimal_total_cost / _G_nx.edges[source, target][_weight])  # Divided by the length of d(i, j)
...
def _compute_ricci_curvature_edges(G: nx.Graph, weight="weight", edge_list=[],
    ...
    global _apsp
    global _G_nx # 新增
    global _nx_nodes_to_idx # 新增
    global _nx_idx_to_nodes # 新增
    # -------------------------------------------------------

    # _Gk = nk.nxadapter.nx2nk(G, weightAttr=weight) # 移除
    _G_nx = G.copy() # Copy graph to global variable for worker processes
    _alpha = alpha
    _weight = weight
    _method = method
    _base = base
    ...
    _nbr_topk = nbr_topk
    # _nx_nodes_to_idx, _nx_idx_to_nodes = {}, {} # Initialize inside the function before APSP

    # The _nx_nodes_to_idx/_nx_idx_to_nodes mapping is now handled inside _get_all_pairs_shortest_path
    # and is accessible globally after that function runs.
    ...
    # Note: Using 'fork' context is often problematic with NetworkX and shared state.
    # 'spawn' or 'forkserver' might be safer depending on your OS and Python version.
    # However, 'spawn' might require refactoring to avoid pickling issues with global state.
    # For now, keeping 'fork' as suggested in the original comment if it worked previously.
    # If issues arise, consider running sequentially or refactoring for process isolation.
    # with mp.get_context('spawn').Pool(processes=_proc) as pool: # This might cause issues with global state
    # Using default context, which is 'fork' on Unix.
    with mp.Pool(processes=_proc) as pool:
        # WARNING: Now only fork works, spawn will hang. (Original comment)
        # WARNING: Using global state like this with multiprocessing can be risky.
    ...
    output = {}
    for rc in result:
        output.update(rc) # rc is already in the format {(node1, node2): {...}}
...
def _compute_ricci_flow(G: nx.Graph, weight="weight",
    ...
    **kwargs
    ...
    global _apsp
    global _G_nx # Need to update global _G_nx if G changes significantly
    ...
    if nx.get_edge_attributes(G, "original_RC"):
        ...
        _apsp = {}
        _G_nx = G.copy() # Update global graph if needed for subsequent steps within this function's context

    # Start the Ricci flow process
    for i in range(iterations):
        for (v1, v2) in G.edges():
            # Update weight based on current Ricci curvature
        ...
        sumw = sum(w.values())
        if sumw > 0: # Prevent division by zero
            for k, v in w.items():
                w[k] = w[k] * (normalized_weight / sumw)
        else:
            logger.warning("Sum of weights is zero after Ricci flow update. Stopping normalization.")
            break
        ...
        if rc: # Check if rc is not empty
            diff = max(rc.values(), key=lambda x: x['rc_curvature'])['rc_curvature'] - min(rc.values(), key=lambda x: x['rc_curvature'])['rc_curvature']

            logger.trace("Ricci curvature difference: %f" % diff)
            # Note: max/min on dict values needs to access the 'rc_curvature' key
            rc_vals = [v['rc_curvature'] for v in rc.values()]
            w_vals = list(w.values())
            if rc_vals and w_vals:
                logger.trace("max_RC:%f, min_RC:%f | maxw:%f, minw:%f" % (
                    max(rc_vals), min(rc_vals), max(w_vals), min(w_vals)))
        else:
            logger.warning("No ricciCurvature values found in edges after iteration %d." % i)
            break

        if rc and diff < delta:
        ...
        if i != 0 and i % do_surgery == 0:
            ...
            normalized_weight = float(G.number_of_edges())
            _G_nx = G.copy() # Update global graph if surgery changes it
        ...
        # clear the APSP since the graph have changed.
        _apsp = {}
        _G_nx = G.copy() # Update global graph for next iteration's APSP calculation if needed
